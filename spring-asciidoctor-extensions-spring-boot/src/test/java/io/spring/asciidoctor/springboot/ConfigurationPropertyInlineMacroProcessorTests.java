...
public class ConfigurationPropertyInlineMacroProcessorTests {
	...
	@Test
	public void whenPropertyThatExistsIsReferencedConversionSucceeds() {
	...
	public void whenPropertyThatDoesNotExistIsReferencedAWarningIsLogged() {
		...
		assertThat(this.logRecords).hasSize(1);
		LogRecord logRecord = this.logRecords.get(0);
		assertThat(logRecord.getSeverity() == Severity.WARN);
		assertThat(logRecord.getMessage()).isEqualTo("Configuration property 'does.not.exist' not found.");
	}

	@Test
	public void whenPropertyThatIsDeprecatedIsReferencedAsDeprecatedConversionSucceeds() {
	...
	public void whenPropertyThatIsDeprecatedIsReferencedAWarningIsLogged() {
		...
		LogRecord logRecord = this.logRecords.get(0);
		assertThat(logRecord.getSeverity() == Severity.WARN);
		assertThat(logRecord.getMessage()).isEqualTo("Configuration property 'example.property.bravo' is deprecated.");
	}
	...
	public void whenPropertyThatIsNotDeprecatedIsReferencedAsDeprecatedAWarningIsLogged() {
		...
		assertThat(logRecord.getMessage())
				.isEqualTo("Configuration property 'example.property.charlie' is not deprecated.");
	}
